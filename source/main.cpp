...
int main() {
    ...
    // Scan latest-log.txt for gametype names and write for Lua script
    {
        auto gt_data = bridge_get_gametypes_from_log();
        if (!gt_data.empty()) {
            std::string gt_dir = srb2_dir + "/client/DiscordBot";
            std::filesystem::create_directories(gt_dir);
            std::ofstream gt_file(gt_dir + "/gametypes.lua", std::ios::trunc);
            gt_file << "-- Generated by srb2dbot from latest-log.txt\n";
            gt_file << "return {\n";
            for (const auto& [gt, name] : gt_data) {
                gt_file << "    [" << gt << "] = \"" << name << "\",\n";
            }
            gt_file << "}\n";
            std::cout << "[init] Wrote " << gt_data.size() << " gametype names from log" << std::endl;
        }
    }
