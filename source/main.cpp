...
int main() {
    ...
    // Write gametypes.lua with display names from WAD file scanning
    {
        auto gt_data = bridge_scan_pk3_gametypes();
        if (!gt_data.empty()) {
            std::ofstream gt_file(bridge_dir + "/gametypes.lua", std::ios::trunc);
            gt_file << "-- Generated by srb2dbot - custom gametype identifiers and display names\n";
            gt_file << "return {\n";
            for (const auto& [id, name] : gt_data) {
                gt_file << "    [\"" << id << "\"] = \"" << name << "\",\n";
            }
            gt_file << "}\n";
            std::cout << "[init] Wrote " << gt_data.size() << " gametype mappings to gametypes.lua" << std::endl;
        } else {
            std::cout << "[init] No custom gametype mappings found in PK3s" << std::endl;
        }
    }
